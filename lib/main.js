...
const START_TAB_ID = IDENTIFIER+"StartingTab" // attached to the very first tab
const MAX_NUM_SEARCHES = 20;
...
function addToLogfile(logData) {
	...
	Task.spawn(function () {
		...
		let pfh = yield OS.File.open(pathToLogFile, exists ? {
				read : true,
				write : true,
				existing : true,
				append : true
...
TABS.on("ready", function (tab) { // listen for a new url's page to be loaded
	...
	checkLoginPage(tab).then(function onSuccess(truthValue){
			if(truthValue && !foundCredentials){
					console.log("main-js:*************************************************************");
					console.log("main-js: URL: "+tab.url);
					console.log("main-js: Is a Login Page");
					console.log("main-js:*************************************************************");
					listenForLoginSubmit_3(tab);
					...
					console.log("main-js:*************************************************************");
					console.log("main-js: URL: "+tab.url);
					console.log("main-js: Not a Login Page");
					console.log("main-js:*************************************************************");
			}
...
TABS.on("close", function(tab){
		searchForTag(tab, START_TAB_ID)
		.then(function(truthValue){
				if(truthValue){
						loggedInFlag = false;
				}
		});
...
function searchForTag(tab, identifier) {
	...
	//console.log("main-js: On TAB: "+tabURL.toString().slice(0,100)+"...";
	//console.log("\n");
...
function listenForLoginSubmit(tab) {
	...
	if (hostURL == null) {
		hostURL = fullTabURL.host;
	}

	console.log("main-js: 1.Current Tab: " + fullTabURL.host);
			...
			tab.on("ready", function () {
				if (!scriptRunning) {
					...
					require("sdk/timers").setTimeout(function () {
						updatePassword(tab, hostURL, siteURL, formSubmitURL, enteredLogin, loginField, enteredPassword, passwordField, 0, true, true, true, true);
					}, 3000);
					
					console.log("main-js: Failed to Change Password after "+MAX_NUM_SEARCHES+" searched. Closing all tabs.");
					closeCloseableTabs();
...
function listenForLoginSubmit_3(tab){
	var fullTabURL = require("sdk/url").URL(tab.url);
	var siteURL = fullTabURL.protocol+"//"+fullTabURL.host+"/";
	...
	.then(function onSuccess(truthValue){
		...
		if(!loggedInFlag){
			storedCredentials = [];
			getStoredCredentials(tab, siteURL)
			.then(function onSuccess(returnedObj){
				if(returnedObj.truthValue){
						
						
					require("sdk/timers").setTimeout(function () {
						console.log("main-js: ******* CALL updatePassword() ******** ");
						updatePassword(tab, hostURL, siteURL, returnedObj.FormSubmitURL, returnedObj.Username, returnedObj.UsernameField, returnedObj.EnteredPassword, returnedObj.PasswordField, 0, true, true, true, true);
					}, 3000);
	...
	.then(null, function onError(error){
		console.log("main-js: loginSubmitListener: searchForTag(tab, NO_LOGIN_LISTENER_ID) ERROR - "+error);
	});
}

// get the stored username and password from password manager
function getStoredCredentials(tab, siteURL){
		var deferred = defer();
		...
		passwords.search({
				...
				onComplete: function onComplete(credentials){
					for(var i=0; i<credentials.length; i++){
						storedCredentials.push(credentials[i]);
					}
					if(storedCredentials.length == 0 ){
						console.log("main-js: loginSubmitListner_3: No password Stored for this website: "+siteURL);
						var result = {truthValue: false};
						deferred.resolve(result);
					}
					else if(storedCredentials.length == 1){
						console.log("main-js: loginSubmitListner_3: Greate found  User Credentials for this password. Moving ahead...");		
						var formSubmitURL = storedCredentials[0].formSubmitURL;
						var enteredLogin = storedCredentials[0].username;
						var loginField = storedCredentials[0].usernameField;
						var enteredPassword = storedCredentials[0].password;
						var passwordField = storedCredentials[0].passwordField;
						console.log("main-js: ****************************************************************** ");
						console.log("main-js: Username: "+enteredLogin);
						console.log("main-js: Password: "+enteredPassword);
						console.log("main-js: URL: "+storedCredentials[0].url);
						console.log("main-js: formSubmitURL: "+formSubmitURL);
						console.log("main-js: UsernameField: "+loginField);
						console.log("main-js: PasswordField: "+passwordField);
						console.log("main-js: ****************************************************************** ");
						
						var result = {truthValue: true, 
						...
						console.log("mian-js: loginSubmitListner_3: More than 1 credentials for this website: "+siteURL);
						var result = {truthValue: false};
						deferred.resolve(result);
					}
...
function updatePassword(inputTab, hostURL, siteURL, formSubmitURL, enteredLogin, loginField, enteredPassword, passwordField, count, continueSearching, needCheckLogin, needCheckStored, needGetResetURL) {

	console.log("main-js: Recursive COUNT: " + count);
	console.log("main-js: CURRENT TAB: " + inputTab.url);
	console.log("main-js: HOST URL: " + hostURL);
	console.log("main-js: Site URL: " + siteURL);
	/* if(tabStack.length ==0){
	console.log("TAB STACK IS NULL");
	}else{
	console.log("TAB STACK CONTENT");
	for(var i=0; i<tabStack.length; i++){
	console.log(tabStack[i].url);
	}
	} */
	...
	promise.then(function () {
		if (needCheckLogin) {
			...
			.then(function onSuccess(loggedIn) {
				...
				attachIdentifierToPage(tab, START_TAB_ID);
				if (!loggedIn) {
	...
	.then(function (newResetURL) {
		if (needGetResetURL) {
			...
			if (newResetURL == null) {
				return getResetURL(tab, hostURL, searchHistory);
			} else {
	...
	.then(function onSuccess(newResetURL) {
		...
		if (newResetURL != null && newResetURL.length > 0 && searchHistory.indexOf(newResetURL) < 0 && newResetURL.indexOf("newNewURL") < 0) {
			...
			console.log("main-js: updatePassword: Search produced no new URL. Pop-tab form stack");
			if (tabStack.length != 0) {
				var curTab = tab;
				tab = tabStack.pop(); // take parent tab from stack
				resetTabRemoveIdentifiers(tab);
				resetURL = null; // to make suer we don't open any new tab.
				curTab.close(); // close child tab, since there are no new reset URLS
			} else {
				continueSearching = false;
				throw new Error("main-js: updatePassword: Stack EMPTY, no more tabs to search. Abandoning Search");
			}
	...
	.then(function onSuccess(newTab) {
		...
		if (resetURL != null && resetURL.indexOf("twitter.com/") > -1) {
			...
			delay = 2000;
			//console.log("main-js: Waiting 2 sec to executeResetScript on newTab, Since its Twitter");
		}
	...
	.then(function onSuccess(output) {
		...
		.then(function onSuccess(truthValue) {
			if (truthValue) {
				 ...
				 * however, 
				 * Amazon requires a form submission so storing a URL won't work for that site
				 * Storing URL also won't work for nytimes.com
				 */
				if (siteURL.indexOf("amazon.com") < 0|| siteURL.indexOf("nytimes.com")<0) {
		...
		.then(function onSuccess() {
			...
			updatePassword(tab, hostURL, siteURL, formSubmitURL, enteredLogin, loginField, enteredPassword, passwordField, count, continueSearching, needCheckLogin, needCheckStored, needGetResetURL);
		})
		.then(null, function onFailure(error) {
...
function checkLogin(tab, siteURL) {
	var deferred = defer();
	let worker = tab.attach({
	...
	worker.port.emit("start", siteURL);
...
function checkLoginPage(tab){
		var deferred = defer();
		var worker = tab.attach({
				contentScriptFile : DATA.url("isLoginForm-Script.js")
		});
		...
		worker.port.on("true", function(){
				deferred.resolve(true);
		});
		
		worker.port.on("false", function(){
			deferred.resolve(false);
		});
...
function getResetURL(tab, hostURL, searchHistory) {
	var deferred = defer();
	...
	.then(function onSuccess() {
		worker.port.emit("start", hostURL, CLOSEABLE_TAB_ID, FORM_SUBMITTED_ID, searchHistory);
	})
	.then(null, function onFailure(error) {
		reportMessage(error.fileName + ":" + error.lineNumber + ": " + error);
	});
	
	worker.port.on("foundButton", function(){
		/* Add delay to let the page load after clicking the button*/
		require("sdk/timers").setTimeout(function () {
			console.log("main-js: Reset URL found in BUTTON : " + tab.url.toString().slice(0, 100) + "...");
			deferred.resolve();
		}, 5000);
	});

	worker.port.on("foundForm", function (returnedFormURL) {

		searchHistory.push(returnedFormURL);
		...
		require("sdk/timers").setTimeout(function () {
			console.log("main-js: Reset URL found in FORM : " + tab.url.toString().slice(0, 100) + "...");
			deferred.resolve();
		}, 5000);
	...
	worker.port.on("found", function (returnedURL) {
		console.log("main-js: returnedURL: " + returnedURL.toString().slice(0, 100) + "...");
	...
	worker.port.on("notFound", function () {
		console.log("main-js: No new URL...");
		deferred.resolve("noNewURL");
...
function prepareToExecute(tab, resetURL) {
	...
	.then(function () {
		if (openURLInNewTab && resetURL != null) {
			openNewTab(resetURL)
			.then(function onSuccess(newTab) {
				require("sdk/timers").setTimeout(function () {
					console.log("----------------------> TAB STACK PUSH");
					tabStack.push(oldTab);
					deferred.resolve(newTab);
				}, 2000);

			}) // sends the newly-opened tab to next promise
			.then(null, deferred.reject);
		} else {
...
function openNewTab(urlToOpen) {
	var deferred = defer();
	console.log("main-js: Opening in New Tab: " + urlToOpen.toString().slice(0, 150) + "...");
	var scriptRunning = false;
	TABS.open({
		url : urlToOpen,
		inBackground : true,
		...
		onReady : function () {
			if (!scriptRunning) { // or this will execute over and over because a page sends ready signals after each form submission
				...
				attachIdentifierToPage(tab, NO_LOGIN_LISTENER_ID); // add note to avoid listening to login-submit
				deferred.resolve(tab);
			}
		}
	});
...
function executeResetScript(tab, siteURL, enteredLogin, enteredPassword, searchHistory) {
	console.log("main-js: Attempting to reset pass on " + tab.url.toString().slice(0, 100) + "...");
	var deferred = defer();
	var needsCloseableTag = false;
	...
	.then(function onSuccess() {
		worker.port.emit("start", siteURL, enteredLogin, enteredPassword, getResetEmail(siteURL), NO_LOGIN_LISTENER_ID);
	})
	.then(null, function onFailure(error) {
		reportMessage(error.fileName + ":" + error.lineNumber + ": " + error);
	});
	...
	worker.port.on("wasLoginScreen", function () {
		...
		// wait for few seconds before moving ahead, to let the page load, this avoids checking for successful login on same page, in which case it returns as BAD LOGIN.
		console.log("main-js: Login credentials entered. WAITING for 5 sec for page load");
		require("sdk/timers").setTimeout(function () {
	...
	/* worker.port.on("redyToSubmit", function(returnObj){
			worker.port.emit("submitForm", returnObj);
	}); */
	worker.port.on("finished", function (newPass) {
		...
		require("sdk/timers").setTimeout(function () {
			...
			if (newPass == null) {
				...
				attachIdentifierToPage(tab, NO_LOGIN_LISTENER_ID);
...
function updateStoredResetURLs(siteURL, resetURL) {
	...
	Task.spawn(function () {
		...
		let pfh = yield OS.File.open(fileName, exists ? {
				read : true,
				write : true,
				existing : true,
				append : true
...
function checkReset(tab, siteURL, formSubmitURL, enteredLogin, loginField, newPass, passwordField) {
	...
	worker.port.on("good_login", function () {
		...
		.then(function onSuccess() {
			...
			// required in case of page refresh
			searchForTag(tab, CLOSEABLE_TAB_ID)
			.then(function onSuccess(truthValue) {
				if (!truthValue) {
					console.log("main-js: checkReset() - Attaching CLOSABLE_TAB_ID");
					attachIdentifierToPage(tab, CLOSEABLE_TAB_ID);
				}
				deferred.resolve();
			});
...
function storePassAsync(tab, siteURL, submitURL, myUsername, myUsernameField, myNewPass, myPassField) {
	...
	console.log("main-js: storePassAsync()-submitURL: " + submitURL);
...
// remove specified identifier from tab
function removerIdentifierOnTab(tab, identifier) {

	let worker = tab.attach({
			contentScriptFile : DATA.url("remove-meta-tag.js")
		});
...
// Reset the tab after popping it from tabs Stack. Remove specified tags
function resetTabRemoveIdentifiers(tab) {
	removerIdentifierOnTab(tab, CREATE_NEW_TAB_ID);
	removerIdentifierOnTab(tab, SEARCH_AGAIN_ID);
	removerIdentifierOnTab(tab, LOGIN_AGAIN_ID);
}
